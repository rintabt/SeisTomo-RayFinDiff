...
while Temp.src_ke < N.src % Forward (per-src) & Telusur raypath (per-rec)
    ...
    if Pilih.IF == 2 && Pilih.display_gambar ~=0 %Plot travel time (2D)
        ...
        figure(Indeks.fig)
        set(gcf,'Units','Normalized','OuterPosition',[0.01 0.02 0.94 0.95])
        if Pilih.display_log==1
            set(gcf,'Units','Normalized','OuterPosition',[0.01 0.02 0.7 0.95])
        end
        subplot(2,2,[1 3])
        plot_TimeForward(Model, Temp, Ray.time_2D{Temp.src_ke})
        axis image, hold on
...
%             disp '~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'
        ...
        if Pilih.IF == 2 && Pilih.display_gambar == 1 %Plot ray per rec-src
            plot(Temp.KoordRay(:,1),Temp.KoordRay(:,2), 'LineWidth', 1.5)
            pause(0.1)
        end
    ...
    if Pilih.display_gambar == 1
        ...
        while Temp.src_ke < N.src
            ...
            if Pilih.SaveGambar == 1
                Temp.savenama = [Pilih.Tempat Model.NamaSave '_source-' num2str(Temp.src_ke) '.jpg'];
